...
通过 new XMLHttpRequest() 建立一个 http 请求对象；
open 函数的作用是设置要打开的 url 和类型，建立一个连接，但此时请求并没有发送；
setRequestHeader 来设置请求头信息；
send 函数像服务器发送数据请求；
onreadystatechange 是一个监听函数，当 readyState 改变的时候执行，1-2-3-4，4 表示成功返回。xhr.responseText 是返回的响应数据，很明显，这里是 json 格式，实际要通过响应头来判断，这里省去了这一步，getAllResponseHeaders 可以获得所有响应头；
success 函数和 complete 函数执行的位置和顺序问题。
jQuery ajax 的特点
